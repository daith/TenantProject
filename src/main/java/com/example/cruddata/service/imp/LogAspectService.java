...
public class LogAspectService {
    ...
    public Object logging(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        long elapsedTime = responseTime- startTime;
        String remoteAddrIP = request.getRemoteAddr();
        String methodType = request.getMethod();
        String url = request.getRequestURL().toString();
        String header = getHeaderInfo(request);

        logger.info("[{}] [{}] [{}] [{}] [{} ms]", remoteAddrIP, methodType, url, class_method, elapsedTime);
        logger.info("-----------------REQUEST------------------");
        ...
        logData.setMethodType(methodType);
        logData.setUrlData(url);
        logData.setController(class_method);
        logData.setRemoteAddrIp(remoteAddrIP);
        logData.setEntryTime(new Date(startTime));
        logData.setResponseTime(new Date(responseTime));
        logData.setHeader(header);
        logData.setResponse(mapper.writerWithDefaultPrettyPrinter().writeValueAsString(responseBody));
        logData.setParameterData(mapper.writerWithDefaultPrettyPrinter().writeValueAsString(request.getParameterMap()));
        logData.setRequestBody(getPayload(request));
        logData.setWasteTime(elapsedTime);
